...
class Batch implements BatchInterface
{
	...
	public function getTraversableList(
		...
		$firstItem = reset($firstResultData);

		if (!array_key_exists('id', $firstItem) && !array_key_exists('ID', $firstItem)) {
			throw new InvalidArgumentException('Return array must contain ID or id key');
		}

		if ($totalElementsCount <= self::MAX_ELEMENTS_IN_PAGE) {
			foreach ($responseInfo[''] as $cnt => $listElement) {
				$elementsCounter++;
		...
		$lastPageResult = $lastResultPage->getResponseData()->getResult()->getResultData();
		$lastElementId = (int)reset($lastPageResult)['ID'];
		...
		foreach ($this->getTraversable(true) as $queryCnt => $queryResultData) {
			...
			foreach ($queryResultData->getResult()->getResultData()['result'] as $cnt => $listElement) {
				$elementsCounter++;
				if ($limit !== null && $elementsCounter > $limit) {
					return;
				}
	...
	protected function getTraversable(bool $isHaltOnError): Generator
	{
		...
		foreach ($this->getTraversableBatchResults($isHaltOnError) as $batchItem => $batchResult) {
			...
			$resultDataItems = $response->getResult()->getResultData();
			...
			foreach ($resultDataItems['result'] as $singleQueryKey => $singleQueryResult) {
	...
	private function getResponseInfo(string $apiMethod, Response $response): array
	{
		$resultData = $response->getResponseData()->getResult()->getResultData();

		return [
			'firstItem' => reset($resultData),
			''
		];
	}
}
